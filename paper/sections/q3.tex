...
问题三在固定电价条件下，每天00、06、12、18点可获得未来整点的光伏预报，并允许在后三个时刻修订尚未交付的购电计划。新增预报有助于更新对光伏出力的判断，日内调整则可以结合实际库存和近期负荷重新安排购电。为区分预报更新与调整机制的作用，后文将在相同调整时点下比较不同预报信息的实际效果。
...
\end{equation}
本式采用$q^{(s)}_{d,t}\ge g_{d,t}$的约束，具体原因见下节。与问题二一致，未来紧急购电风险仍然通过历史净负荷误差分位进行修正，最终费用则依据实际运行结果核算。

附件三中，$h$点发布的第$k$列预报对应$h+k$时刻。每次收到预报后，以最近一个已完成区间的光伏观测为插值起点，依次连接未来整点值，得到十分钟预测$\widehat P^{\mathrm{V,F}}$，并取其中截至当日24点的部分。

为兼顾历史规律与最新预报，采用线性加权融合：
\begin{equation}
...
\begin{equation}
 ...
 \right),\quad t\ge s.
\end{equation}
横线表示索引$j=s-18,\ldots,s-1$上的算术均值，分母中的1以kW计，用于避免预测均值过小导致比值异常。该比值反映最近实际负荷与原预测的偏差。按式\eqref{eq:clip}限制在$[0.75,1.25]$后，使单次修正幅度至多为原预测的25\%；上下限由控制规则设定，未作置信水平解释。在此基础上，将修正后的负荷预测与融合光伏预测作差得到净负荷点预测，再依据式\eqref{eq:risk}叠加对应发布时间的历史残差分位。不同$\lambda$及预报组合分别建立残差样本，且全部仅使用此前日期的数据，以保证预测与参数估计过程满足信息时序约束。
...
\begin{enumerate}[label=步骤\arabic*,leftmargin=*,align=left]
 \item 零时继承真实库存，读取已发布光伏预报，并与历史曲线融合后作风险修正，求解全天原计划$g$。
 \item 在06、12、18时读取最新光伏预报及已完成时段的负荷、光伏观测，更新剩余时段预测。
 \item 以当前实际库存为初始状态，求解$(\mathrm P_3^{(s)})$，修订尚未交付的购电计划并记录调整结果。
 \item 各交付时段以当前$q_t$作为常规购电量，按照问题二的反馈规则执行储能调度，并在必要时进行紧急购电。
 \item 根据最终$q_t-g_t$计算主合约调整费，日末将实际库存继续传递至次日。
\end{enumerate}
...
表\ref{tab:q3-days-summary}汇总四个指定日期，正文按题面格式列出9月23日，其他日期及紧急事件、账单见附录\ref{app:q3-days}，并与\texttt{result3.xlsx}保持一致。购电表分别给出零时原计划与最终常规购电量，两者差值即为净调整量。其中，原计划部分的“全天购电费”为原计划费，在最终部分为原计划费加调整费，均不含紧急费；储能表为实际执行值。
\input{generated/q3-days-summary}
\input{generated/representative-day-q3}
9月23日的紧急购电量从问题二的892.40\,kWh降至170.85\,kWh，总费用相应减少3909.92元；但3月20日的日内方案比问题二多支出109.40元，说明调整机会并不保证每一天都降低费用。
...
\FloatBarrier
读取全部新预报较完全不读取新预报可节省17225.82元。分别撤去06时或12时的新预报会增加费用，而撤去18时预报仅变化$-0.31$元。说明在本组参数设定下，06时和12时预报有助于降低费用，18点的新预报收益很小。各次预报的影响随库存和后续计划相互关联，单项费用差不能直接相加。对照仍保留18点调整，可利用当时的负荷观测和库存变化重新安排购电。
...
\FloatBarrier
逐笔成交不可撤回时，重新选参和优化的总费用为\PaperValue{q3-per-trade-cost}\,元，仍较问题二低447673.33元，但节省幅度小于主合约。附录\ref{sec:contracts}分别给出原调度方案重新计费与重新优化后的结果。对比表明，调整费用的累计方式会直接影响日内调度的经济收益。
\label{end:q3}
