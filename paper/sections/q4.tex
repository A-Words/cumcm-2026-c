...
本文将“交易时刻电价”解释为对应交付区间的实际价格$p_{d,t}$，在交付和结算时使用。沿用问题三的交易变量，记$\widehat p^{(s)}_{d,t}$为时刻$s$根据已有信息预测的未来价格。问题4-2在$s=0$制定全天固定计划；问题4-3在$s\in\{0,36,72,108\}$规划，并沿用光伏预报融合及最终净调整合约。

两种策略采用相同设备参数、十分钟步长和充放电反馈，实际库存均随每日运行连续传递。
...
价格预测同样利用每周规律。对$d\ge7$，取此前与目标日星期相同的日期，对同刻价格加权，得到
\begin{equation}
 \overline p_{d,t}=
